...
method_set = ['FGSM' ,'IFGSM', 'PGD', 'ERM',  'BayesWRM', 'Bayes']
...
for rind in range(rounds):
    for method in method_set:
        if method == 'BayesWRM':
            ...
            os.system('python main_advtrain.py  --eps=5e2 --advtraining='+method) 
        elif method == 'FGSM':
            os.system('python main_advtrain.py  --epsilon=0.02 --advtraining='+method) 
        elif method == 'IFGSM':
            os.system('python main_advtrain.py  --epsilon=0.0014 --advtraining='+method) 
        elif method =='PGD':
            os.system('python main_advtrain.py  --epsilon=1e6 --advtraining='+method) 
        else:
