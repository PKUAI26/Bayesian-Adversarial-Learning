...
def adv_train(X, y, model, criterion, adversary):
    ...
    if type(model) is list:
        for ii in range(len(model)):
            model_list_new.append(copy.deepcopy(model[ii]))
            for p in model_list_new[ii].parameters():
                p.requires_grad = False
            model_list_new[ii].eval()
        adversary.model_list = model_list_new
    else:
    ...
    if type(model) is list: # return a list of numpy arrays
        return X_adv 
    else: # return a torch tensor
